...
preqin_basis$get.factor.predictor.data <- function(fred.vec) {
  ...
  df.ff$Mkt <- df.ff$Mkt.RF + df.ff$RF
  df_pubin <- merge(df.xl, df.ff, by = "Date", all.x = TRUE)
  df_pubin <- df_pubin[, !is.na(df_pubin[df_pubin$Date == as.Date("1984-12-31"), ])]
  #df_pubin <- df.ff
  
  factor.set <- c("Mkt.RF", "SMB", "HML", "RMW", "CMA")
  zero.investment.factors <- c("Mkt.RF", "SMB", "HML", "RMW", "CMA")
  exclude <- c("Date", zero.investment.factors)
  factor.set <- colnames(df_pubin)[!(colnames(df_pubin) %in% exclude)]
...
if(sys.nframe() == 0L) {
  ...
  preqin_basis_list <- readRDS("preqin_basis/RF_valueweighted_preqin_basis.RDS")
  preqin_basis_list$original$VC$predictor.set
  preqin_basis_list$original$VC$factor.set
  df <- preqin_basis_list$original$VC$df$`1986`
}

...
# GLM Net ----
fit <- glmnet(x = as.matrix(df.x[, 2:nrow(df.x)]), y = df.x$Discounted.CF, standardize = FALSE, intercept=FALSE)
plot(fit)
print(fit)
coef(fit, s = 0.1)
coef(fit, s = 0.2)
...
cv.fit <- cv.glmnet(x = as.matrix(df.x[, 2:nrow(df.x)]), y = df.x$Discounted.CF, standardize = FALSE, intercept=FALSE)
plot(cv.fit)
cv.fit$lambda.min
...
formula <- paste("Discounted.CF ~", paste(colnames(df.x)[-1], collapse = " + "), "- 1")
...
system.time(
  k <- ols_step_forward_p(fit, penter = 0.05, details = FALSE, progress = TRUE)
)
k
# plot(k)

start_lm <- lm(Discounted.CF ~ Mkt.RF_x_TEDRATE.lag1 + 0, data = df.x)
summary(start_lm)
summary(step(start_lm, direction = "forward", scope = formula(fit), k = log(ncol(df.x)), trace = 1, steps = 5))
